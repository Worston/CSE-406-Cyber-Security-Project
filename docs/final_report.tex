...
Two related password guessing attacks were built and demonstrated against a
signup and login webapp written from scratch, along with a working
defense against both.

\begin{itemize}[leftmargin=*]
  \item \textbf{Dictionary attack.} One specific account is targeted, and a
    long, ordered list of candidate passwords is tried against it, one at a time,
  ...
  \item \textbf{Known password attack} (password spraying, or credential
    reuse). Instead of guessing many passwords against one account, a small
    set of known passwords, such as defaults or passwords leaked in earlier
    breaches, is used as the starting point, and each one is tried against a
    wide list of accounts. This exploits password reuse and default
    credentials rather than weak individual passwords.
\end{itemize}

The target is a webapp built from scratch rather than an existing
...
by HTTP client code written from scratch, not an existing brute forcing tool.
Alongside the attacks, a defense mechanism was built, account lockout
...
The attack was run on two ordinary devices connected to the same local network, with no virtual machines involved. The victim device runs the webapp and is reachable at \texttt{192.168.x.a} on port 5000. The attacker device sits on the same network and talks to the victim over plain HTTP. A browser on the same network is used for the legitimate signup and login action.
...
\node[attackerbox] (attacker)
  {\textbf{\color{attackerred}Attacker device} \\ 192.168.x.b \\[4pt]
...
\node[victimbox, right=3.2cm of attacker] (victim)
  {\textbf{\color{victimblue}Victim device} \\ 192.168.x.a \\[4pt]
...
The diagrams below describe the protocol implemented by the webapp and the
attack strategies run against it. Request arrows (client to server) are
...
built-in development server. What was designed from scratch is the
application behaviour and the payload carried by the HTTP request, and how
it is interpreted by the server, not an existing authentication protocol
implementation.
...
\begin{itemize}[leftmargin=*]
  \item Each attempt opens its own TCP connection, since the attacker
    ...
    FIN/ACK teardown, for every attempt.
...
\begin{itemize}[leftmargin=*]
  \item \textbf{Victim device.} Runs the Flask webapp bound to all
    interfaces, reachable at \\
    \texttt{http://192.168.x.a:5000} from the
    rest of the network. Started undefended with \texttt{python seed.py} followed by \texttt{python app.py}, which hashes passwords with a single fast salted SHA-256 and enforces no lockout. Started defended with a fresh \texttt{app.db}, \texttt{python seed.py --defense}, and \texttt{DEFENSE=1 python app.py}, which hashes with salted PBKDF2-HMAC-SHA256 at 200{,}000 iterations and turns the lockout tracker on.
  \item \textbf{Attacker device.} On the same local network as the victim,
    pointed at \\
    \texttt{http://192.168.x.b:5000}. Runs \texttt{attacker\_dict.py} against the account \texttt{alice} using \\
    \texttt{wordlist.txt} (1000 candidate passwords), and \texttt{attacker\_spray.py} against the six accounts in \texttt{targets.txt} using the 33 passwords in \texttt{known\_passwords.txt}.
\end{itemize}
\newpage
...
Before running either attack, the webapp was exercised the way a normal user
would, to confirm the signup and login flow works on its own.

\begin{enumerate}[leftmargin=*]
  \item Open \texttt{http://192.168.x.a:5000/signup} in a browser and
...
\begin{figure}[H]
\centering
\includegraphics[width=0.32\textwidth]{screenshots/signup.png}\hfill
\includegraphics[width=0.32\textwidth]{screenshots/login.png}\hfill
\includegraphics[width=0.32\textwidth]{screenshots/dashboard.png}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item From the attacker device, run \\
    \texttt{python attacker\_dict.py --url http://192.168.x.a:5000} \\
    \hspace{1em}\texttt{--user alice --wordlist wordlist.txt}
  \item One \texttt{POST /login} request is sent by the script per
    candidate password in the wordlist, in order, and the HTTP status code
    is read back.
  \item It stops as soon as \texttt{302} (password found),
    \texttt{429} (locked out) is seen, or the wordlist runs out.
\end{enumerate}
...
\begin{enumerate}[leftmargin=*]
  \item Start the victim webapp, undefended or defended.
  \item From the attacker device, run \\
    \texttt{python attacker\_spray.py --url http://192.168.x.b:5000} \\
    \hspace{1em}\texttt{--userlist targets.txt --passwords known\_passwords.txt}
  \item One known password is tried by the script against every account in
    the target list before moving on to the next known password.
  \item Every attempt is recorded, along with every account where a
    \texttt{302} came back.
\end{enumerate}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{screenshots/attack_run.png}
...
This section is the core of the report. For each attack, in each mode,
what was expected from the Design Report is described, along with what
actually happened according to the logs from the two-device run, and,
where the two do not match exactly, why. All log excerpts below are copied
...
\textbf{What was expected.} Since \texttt{alice}'s password \texttt{sunshine}
is inside \texttt{wordlist.txt}, the attack was expected to find it well
before the list ran out, and to do so quickly, since every hash check is a
single fast SHA-256.

\textbf{What was observed.} The attack found the password on attempt 49 out
of 1000, in about 2 seconds of wall clock time, at roughly 35 to 50
milliseconds per attempt.

\begin{lstlisting}
[  0.041s] attempt=44 user=alice password='soccer' -> HTTP 200
[  0.037s] attempt=45 user=alice password='harley' -> HTTP 200
[  0.045s] attempt=46 user=alice password='batman' -> HTTP 200
[  0.039s] attempt=47 user=alice password='andrew' -> HTTP 200
[  0.043s] attempt=48 user=alice password='tigger' -> HTTP 200
[  0.040s] attempt=49 user=alice password='sunshine' -> HTTP 302
CRACKED user=alice password=sunshine attempts=49 time=1.96s
\end{lstlisting}

\textbf{Match or not.} This matches what was expected. The password was
found inside the wordlist, near the top few percent of it, and the attack
needed only a couple of seconds of real network traffic to succeed. This
same result, attempt 49, was reproduced across several separate runs,
which indicates the behaviour is stable and not a one time fluke.

\subsection{Dictionary attack, defended}

\textbf{What was expected.} With the lockout set to 5 failed attempts inside
30 seconds, an attacker who does not already know the password should get
locked out after the 5th failure, well before reaching attempt 49. Each
attempt was also expected to take noticeably longer because of the slower
PBKDF2 hash.

\textbf{What was observed.} The account was locked out on attempt 6, before
the correct password was ever tried.

\begin{lstlisting}
[  0.112s] attempt=1 user=alice password='123456' -> HTTP 200
[  0.128s] attempt=2 user=alice password='password' -> HTTP 200
[  0.119s] attempt=3 user=alice password='12345678' -> HTTP 200
[  0.131s] attempt=4 user=alice password='qwerty' -> HTTP 200
[  0.124s] attempt=5 user=alice password='123456789' -> HTTP 200
[  0.004s] attempt=6 user=alice password='12345' -> HTTP 429
LOCKED_OUT user=alice attempts=6 time=0.62s
\end{lstlisting}
...
blocked here. On timing, the five attempts that still had a password
checked were measured taking longer, around 100 to 135 milliseconds per
attempt here against roughly 35 to 50 milliseconds in the undefended run,
so about two to three times slower. This is a smaller gap than the
roughly 200{,}000 times more hashing work that PBKDF2 at 200{,}000
iterations actually does per check. The reason is that this measurement is
the whole HTTP round trip across the network, which also includes the
Flask development server and Python, not the hash function alone. The
extra hashing cost is real, but here it is an addition on top of a larger
fixed cost. A cleaner way to see the full effect would be timing the hash
function directly rather than the full request. The 6th attempt, which
came back \texttt{429}, is faster than either of these, only a few
milliseconds, since the lockout check happens before the password is
hashed at all: once an account is locked, the server never touches the
password hash again until the lockout expires.

\subsection{Known password attack, undefended}

\textbf{What was expected.} Since \texttt{eve} and \texttt{frank} share the
password \texttt{admin123}, and none of \texttt{alice}, \texttt{bob},
\texttt{carol}, or \texttt{dave} use any password from
\texttt{known\_passwords.txt}, the first pass of the spray attack was
expected to compromise exactly those two accounts and nothing else.

\textbf{What was observed.} Both accounts were compromised on the very first
...
\begin{lstlisting}
[  0.043s] user=alice password='admin123' -> HTTP 200
[  0.039s] user=bob password='admin123' -> HTTP 200
[  0.046s] user=carol password='admin123' -> HTTP 200
[  0.037s] user=dave password='admin123' -> HTTP 200
[  0.041s] user=eve password='admin123' -> HTTP 302
CRACKED user=eve password=admin123
[  0.044s] user=frank password='admin123' -> HTTP 302
...
\textbf{What was expected.} Based on the Design Report, the lockout was
expected to throttle this attack per account, the same way it throttles the
dictionary attack.

\textbf{What was observed.} \texttt{eve} and \texttt{frank} were compromised
...
\begin{lstlisting}
[  0.003s] user=alice password='admin123' -> HTTP 429
[  0.121s] user=bob password='admin123' -> HTTP 200
[  0.128s] user=carol password='admin123' -> HTTP 200
[  0.116s] user=dave password='admin123' -> HTTP 200
[  0.119s] user=eve password='admin123' -> HTTP 302
CRACKED user=eve password=admin123
[  0.125s] user=frank password='admin123' -> HTTP 302
CRACKED user=frank password=admin123
[  0.004s] user=alice password='password1' -> HTTP 429
        ...
[  0.003s] user=alice password='Welcome1' -> HTTP 429
[  0.004s] user=bob password='Welcome1' -> HTTP 429
[  0.003s] user=carol password='Welcome1' -> HTTP 429
[  0.004s] user=dave password='Welcome1' -> HTTP 429
\end{lstlisting}

\textbf{Match or not, and why it differs.} This is the one place where the
actual result does not match what was expected in the Design Report, and it
is worth explaining clearly. The lockout only ever counts a \emph{failed}
attempt. For \texttt{eve} and \texttt{frank}, the very first password tried
against them is already the correct one, so the request that
...
every single password tried against them, which is why they end up locked
out after enough wrong guesses, even though they were never actually
vulnerable to this attack in the first place. \texttt{alice} shows as
locked from the very first line here because a dictionary attack against
her had already used up her five failures moments earlier in the same test
session, so her lockout was still active.

The lesson taken from this is that account lockout, on its own, is a good
...
The attacks and measurements here rest on a number of assumptions, listed
here so the results above are read in the right context.

\begin{itemize}[leftmargin=*]
  \item \textbf{The attacker already knows the target usernames.} Both
    attacker scripts take a username or a list of usernames as input. How
    an attacker would first learn valid usernames is not modeled, for
  ...
  \item \textbf{The attacker has direct, unrestricted access to the login
    ...
    upstream rate limiter between the attacker and the webapp in this setup.
  ...
  \item \textbf{The attacker sends requests back to back with no delay of
    its own.} The scripts do not deliberately slow themselves down or
    ...
    which the current lockout defense, with its fixed 30 second window,
  ...
  \item \textbf{The lockout tracker lives in the memory of a single Flask
    ...
    than this single-process test shows.
  \item \textbf{The slow hash defense is aimed at a different threat than
    the one directly tested here.} It was tested through the login endpoint,
    ...
    lockout in the way at all. That offline scenario was not run here,
...
Two defenses were built into the webapp, both controlled by one
...
Based on the observations above, the two parts of the defense are assessed
separately.
...
accounts that were never actually vulnerable, does not match the original
expectation \\
...
fully deliver on what was expected for the spray attack. That gap is not a
bug in the implementation, it is a real limitation of what a per account
failure counter can ever catch. This is considered a useful finding on its
...
It was shown that a dictionary attack and a known password attack can both
...
exactly the way first expected. The dictionary attack cracked a weak
